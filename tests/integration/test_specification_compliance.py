...
async def test_free_features_compliance():
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            assert (
                job_record["max_attempts"] == 4
            )  # Retry mechanism (retries=3 -> max_attempts=4)
...
async def test_pro_features_compliance():
    ...
    try:
        ...
        high_priority_job = await fastjob.enqueue(
            advanced_job, priority=10, message="high priority", count=1  # High priority
        )

        low_priority_job = await fastjob.enqueue(
            advanced_job, priority=100, message="low priority", count=1  # Low priority
        )
...
async def test_dsl_usage_examples_compliance():
    ...
    try:
        ...
        job_id = await fastjob.enqueue(
            send_email, to="abc@example.com", subject="Hi", body="Body"
        )
        ...
        async with pool.acquire() as conn:
            ...
            expected_job_name = (
                "tests.integration.test_specification_compliance.send_email"
            )
...
async def test_project_structure_compliance():
    ...
    assert os.path.isfile(
        initial_migration
    ), "Initial migration missing: db/migrations/001_initial_schema.sql"
