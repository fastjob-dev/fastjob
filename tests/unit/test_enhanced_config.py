...
from fastjob.config import configure as enhanced_configure
from fastjob.config import (
    get_config,
    get_database_url,
    get_setting,
...
class TestEnhancedConfiguration:
    ...
    def test_environment_missing_database_url(self):
        ...
        # Temporarily remove FASTJOB_DATABASE_URL if it exists
        ...
        try:
            ...
            if original_url:
                os.environ["FASTJOB_DATABASE_URL"] = original_url
    ...
    def test_environment_invalid_values(self):
        ...
        with pytest.raises(
            FastJobConfigError, match="Invalid value.*FASTJOB_DEFAULT_CONCURRENCY"
        ):
            config.from_env()
    ...
    def test_configuration_precedence(self):
        ...
        enhanced_configure(
            database_url="postgresql://localhost/test", worker_concurrency=4
        )
    ...
    def test_get_setting_function(self):
        ...
        enhanced_configure(
            database_url="postgresql://localhost/test", worker_concurrency=8
        )
    ...
    def test_logging_configuration(self):
        ...
        with patch("logging.getLogger") as mock_get_logger:
            ...
            enhanced_configure(
                database_url="postgresql://localhost/test", log_level="DEBUG"
            )
    ...
    def test_log_level_validation(self):
        ...
        for level in valid_levels:
            reset_config()
            enhanced_configure(
                database_url="postgresql://localhost/test", log_level=level
            )
        ...
        with pytest.raises(FastJobConfigError, match="log_level must be one of"):
            enhanced_configure(
                database_url="postgresql://localhost/test", log_level="INVALID"
            )
...
def fastjob_test_config():
    ...
    reset_config()  # Reset any existing configuration first
    enhanced_configure(
