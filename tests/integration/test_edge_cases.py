...
async def test_malformed_job_data():
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            """,
                uuid.uuid4(),
                "tests.test_edge_cases.test_simple_job",
                '{"invalid": "structure"}',
                3,
                100,
                "default",
        ...
        async with pool.acquire() as conn:
            failed_jobs = await conn.fetch(
                "SELECT * FROM fastjob_jobs WHERE status IN ('failed', 'dead_letter')"
            )
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            job_record = await conn.fetchrow(
                "SELECT * FROM fastjob_jobs WHERE id = $1", uuid.UUID(valid_job_id)
            )
...
async def test_exception_handling_in_jobs():
    ...
    try:
        ...
        value_error_job = await fastjob.enqueue(
            exception_job, exception_type="value_error"
        )
        type_error_job = await fastjob.enqueue(
            exception_job, exception_type="type_error"
        )
        runtime_error_job = await fastjob.enqueue(
            exception_job, exception_type="runtime_error"
        )
        unknown_error_job = await fastjob.enqueue(
            exception_job, exception_type="unknown"
        )
        ...
        async with pool.acquire() as conn:
            failed_jobs = await conn.fetch(
                "SELECT * FROM fastjob_jobs WHERE status = 'dead_letter'"
            )
...
async def test_large_job_payloads():
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            job_record = await conn.fetchrow(
                "SELECT * FROM fastjob_jobs WHERE id = $1", uuid.UUID(large_job_id)
            )
            assert job_record["status"] == "done"
...
async def test_registry_edge_cases():
    ...
    original_registry_size = len(_registry)
...
async def test_timezone_and_datetime_handling():
    ...
    try:
        ...
        naive_job_id = await fastjob.enqueue(
            simple_job_local, scheduled_at=naive_time, message="naive datetime"
        )
        ...
        async with pool.acquire() as conn:
            naive_record = await conn.fetchrow(
                "SELECT * FROM fastjob_jobs WHERE id = $1", uuid.UUID(naive_job_id)
            )
            tz_record = await conn.fetchrow(
                "SELECT * FROM fastjob_jobs WHERE id = $1", uuid.UUID(tz_job_id)
            )
