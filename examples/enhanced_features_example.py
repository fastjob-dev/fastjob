...
@fastjob.job(
    retries=1, priority=10, queue="reports", unique=True, args_model=ReportArgs
)
...
async def demonstrate_enhanced_features():
    ...
    job2_duplicate = await fastjob.enqueue(
        generate_daily_report,
        report_type="sales",
        start_date="2025-01-01",
        end_date="2025-01-31",
    ...
    job3 = await fastjob.enqueue(process_payment, payment_id="PAY123", amount=99.99)
    print(f"   💳 Payment job queued: {job3}")

    job4 = await fastjob.enqueue(flaky_task, task_id="TASK001", should_fail=False)
    print(f"   🔄 Flaky task queued: {job4}")

    # Enqueue a job that will fail
    job5 = await fastjob.enqueue(flaky_task, task_id="TASK002", should_fail=True)
    ...
    if job_status:
        print(
            f"   📧 Email job status: {job_status['status']} (priority: {job_status['priority']})"
        )
    ...
    for queue in queue_stats:
        print(
            f"      {queue['queue']}: {queue['total_jobs']} total, {queue['queued']} queued"
        )
    ...
    cancel_job = await fastjob.enqueue(
        flaky_task, task_id="CANCEL_ME", should_fail=False
    )
