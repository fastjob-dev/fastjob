...
class FastJobConfig:
    ...
    def configure(
        ...
        **kwargs
        ...
        config = {
            ...
            'dev_mode': dev_mode if dev_mode is not None else False,  # None = production mode (False)
        }
    ...
    def _validate_config(self, config: Dict[str, Any]) -> None:
        ...
        validations = [
            ('worker_concurrency', lambda x: isinstance(x, int) and x >= 1, 
             "worker_concurrency must be an integer >= 1"),
            ('job_timeout', lambda x: isinstance(x, (int, float)) and x >= 1, 
             "job_timeout must be a number >= 1"),
            ('max_retries', lambda x: isinstance(x, int) and x >= 0, 
             "max_retries must be an integer >= 0"),
            ('default_queue', lambda x: isinstance(x, str) and len(x.strip()) > 0, 
             "default_queue must be a non-empty string"),
            ('poll_interval', lambda x: isinstance(x, (int, float)) and x > 0, 
             "poll_interval must be a number > 0"),
            ('pool_size', lambda x: isinstance(x, int) and x >= 1, 
             "pool_size must be an integer >= 1"),
            ('log_level', lambda x: isinstance(x, str) and x.upper() in 
             ['DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL'], 
             "log_level must be one of: DEBUG, INFO, WARNING, ERROR, CRITICAL"),
            ('dev_mode', lambda x: isinstance(x, bool), 
             "dev_mode must be a boolean"),
        ]
