...
class FastJobError(Exception):
    ...
    def _format_error_message(self) -> str:
        ...
        lines = [
            f"FastJob Error [{self.error_code}]: {self.message}"
        ]
...
class JobValidationError(FastJobError):
    ...
    def __init__(self, job_name: str, validation_errors: List[str], provided_args: Dict[str, Any]):
        context = {
            "job_name": job_name,
            "validation_errors": validation_errors,
            "provided_args": list(provided_args.keys()),
            "provided_values": {k: str(v)[:100] for k, v in provided_args.items()}  # Truncate long values
        }
...
class JobExecutionError(FastJobError):
    ...
    def __init__(
        ...
        if attempt < max_attempts:
            suggestions.append(f"Job will be retried (attempt {attempt + 1}/{max_attempts})")
        else:
            suggestions.append("Job will be moved to dead letter queue after max retries")
...
class QueueError(FastJobError):
    ...
    def __init__(self, operation: str, queue_name: str, reason: str, details: Optional[Dict[str, Any]] = None):
        context = {
            "operation": operation,
...
class WorkerError(FastJobError):
    ...
    def __init__(self, operation: str, reason: str, worker_config: Optional[Dict[str, Any]] = None):
        context = {
            "operation": operation,
            "reason": reason
        }
...
class ConfigurationError(FastJobError):
    ...
    def __init__(self, setting: str, value: Any, reason: str, expected: Optional[str] = None):
        context = {
            "setting": setting,
...
class MigrationError(FastJobError):
    ...
    def __init__(self, migration_step: str, reason: str, database_info: Optional[Dict[str, Any]] = None):
        context = {
...
def create_user_friendly_error(error: Exception, context: Optional[Dict[str, Any]] = None) -> FastJobError:
    """
