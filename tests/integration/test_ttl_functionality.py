...
async def test_ttl_cleanup_removes_expired_jobs(clean_db):
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            cleaned_count = int(cleaned.split()[-1]) if cleaned.split()[-1].isdigit() else 0
            
            assert cleaned_count == 1, f"Should have cleaned up 1 expired job, got {cleaned_count}"
...
async def test_ttl_failed_jobs_not_cleaned_up(clean_db):
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            assert result['status'] == 'dead_letter', "Job should have status 'dead_letter'"
        ...
        async with pool.acquire() as conn:
            ...
            assert result['status'] == 'dead_letter', "Failed job should remain in dead_letter status"
