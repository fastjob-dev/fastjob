...
user_service = FastJob(
    database_url=os.environ.get(
        "USER_SERVICE_DATABASE_URL", "postgresql://localhost/user_service"
    ),
...
payment_service = FastJob(
    database_url=os.environ.get(
        "PAYMENT_SERVICE_DATABASE_URL", "postgresql://localhost/payment_service"
    ),
...
@payment_service.job(
    queue="transactions", priority=1, retries=5, args_model=PaymentData
)
async def instance_process_payment(
...
async def instance_user_signup_workflow(user_id: str, email: str, name: str) -> dict:
    ...
    user_result = await user_service.enqueue(
        instance_process_user_signup, user_id=user_id, email=email, name=name
    )
...
async def demonstrate_global_api():
    ...
    try:
        ...
        payment_job_id = await fastjob.enqueue(
            global_process_payment, payment_id="pay_456", amount=99.99
        )
...
async def main():
    ...
    print(
        "Instance API: Multiple databases, service isolation, production microservices"
    )
