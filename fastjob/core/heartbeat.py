...
class WorkerHeartbeat:
    ...
    def __init__(self, pool: asyncpg.Pool, queues: Optional[List[str]] = None, concurrency: int = 1):
        self.pool = pool
    ...
    async def register_worker(self) -> None:
        ...
        try:
            ...
            logger.info(f"Worker {self.worker_id} registered on {self.hostname}:{self.pid}")
    ...
    async def _heartbeat_loop(self, interval: float) -> None:
        ...
        while not self._stop_heartbeat:
            try:
                ...
                await asyncio.sleep(min(interval, 30))  # Backoff but don't wait too long
...
async def cleanup_stale_workers(pool: asyncpg.Pool, stale_threshold_seconds: int = 300) -> int:
    """
    ...
    try:
        async with pool.acquire() as conn:
            ...
            cleaned_count = int(result.split()[-1]) if result.split()[-1].isdigit() else 0
                ...
                "active_workers": [
                    {
                        ...
                        "uptime_seconds": (datetime.now(timezone.utc) - (row['started_at'].replace(tzinfo=timezone.utc) if row['started_at'].tzinfo is None else row['started_at'])).total_seconds(),
                ...
                "stale_workers": [
                    {
                        ...
                        "pid": row['pid'], 
                        "last_heartbeat": row['last_heartbeat'].isoformat(),
                        "stale_seconds": (datetime.now(timezone.utc) - (row['last_heartbeat'].replace(tzinfo=timezone.utc) if row['last_heartbeat'].tzinfo is None else row['last_heartbeat'])).total_seconds(),
                    }
