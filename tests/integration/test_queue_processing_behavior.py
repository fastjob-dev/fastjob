...
async def test_process_jobs_all_queues(clean_db, result_file):
    ...
    await fastjob.enqueue(write_to_file_job, queue="high", message="job1", result_file=result_file)
    await fastjob.enqueue(write_to_file_job, queue="normal", message="job2", result_file=result_file)
    await fastjob.enqueue(write_to_file_job, queue="low", message="job3", result_file=result_file)
...
async def test_process_jobs_single_queue(clean_db, result_file):
    ...
    # Enqueue jobs in different queues
    await fastjob.enqueue(write_to_file_job, queue="target", message="target_job", result_file=result_file)
    await fastjob.enqueue(write_to_file_job, queue="other", message="other_job", result_file=result_file)
...
async def test_process_jobs_multiple_queues(clean_db, result_file):
    ...
    # Enqueue jobs in different queues
    await fastjob.enqueue(write_to_file_job, queue="queue1", message="job1", result_file=result_file)
    await fastjob.enqueue(write_to_file_job, queue="queue2", message="job2", result_file=result_file)  
    await fastjob.enqueue(write_to_file_job, queue="excluded", message="job3", result_file=result_file)
...
async def test_priority_ordering_across_queues(clean_db, result_file):
    ...
    await fastjob.enqueue(write_to_file_job, queue="queueA", priority=100, message="medium_A", result_file=result_file)
    await fastjob.enqueue(write_to_file_job, queue="queueB", priority=50, message="high_B", result_file=result_file)
    await fastjob.enqueue(write_to_file_job, queue="queueA", priority=200, message="low_A", result_file=result_file)
    await fastjob.enqueue(write_to_file_job, queue="queueC", priority=25, message="highest_C", result_file=result_file)
...
async def test_scheduled_jobs_across_queues(clean_db, result_file):
    ...
    await fastjob.enqueue(write_to_file_job, queue="queue1", scheduled_at=future_time, message="scheduled1", result_file=result_file)
    await fastjob.enqueue(write_to_file_job, queue="queue2", scheduled_at=future_time, message="scheduled2", result_file=result_file)
...
async def test_empty_queue_list(clean_db, result_file):
    ...
    await fastjob.enqueue(write_to_file_job, queue="some_queue", message="job1", result_file=result_file)
...
async def test_queue_efficiency_single_query(clean_db):
    ...
    for i in range(3):
        await fastjob.enqueue(write_to_file_job, queue=f"queue{i}", message=f"job{i}", result_file="/tmp/test")
