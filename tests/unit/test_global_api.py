...
class TestGlobalAPI:
    ...
    def test_global_api_isolation_from_instances(self):
        ...
        # Create instance app
        instance_app = FastJob(database_url="postgresql://instance@localhost/instance_db")
        ...
        assert global_registry.get_job("tests.unit.test_global_api.global_job") is not None
        assert global_registry.get_job("tests.unit.test_global_api.instance_job") is None
        
        # Instance app should have instance_job but not global_job
        instance_registry = instance_app.get_job_registry()
        assert instance_registry.get_job("tests.unit.test_global_api.instance_job") is not None
        assert instance_registry.get_job("tests.unit.test_global_api.global_job") is None
    ...
    def test_global_api_exports_correctly(self):
        ...
        expected_exports = [
            "FastJob", "job", "enqueue", "schedule", "run_worker", "configure",
            "is_dev_mode", "has_plugin_feature", "get_plugin_status", "diagnose_plugins"
        ]
    ...
    def test_job_decorator_without_configure(self):
        ...
        job_meta = app.get_job_registry().get_job("tests.unit.test_global_api.default_job")
        assert job_meta is not None