...
class TestPluginSystemResilience:
    ...
    def test_cli_handles_plugin_manager_errors(self):
        ...
        with patch(
            ...
            side_effect=Exception("Plugin manager error"),
        ):
    ...
    def test_core_commands_unaffected_by_plugins(self):
        ...
        for plugin_state in plugin_states:
            if isinstance(plugin_state(), Exception):
                patch_target = patch(
                    "fastjob.plugins.get_plugin_manager", side_effect=plugin_state()
                )
            else:
                patch_target = patch(
                    "fastjob.plugins.get_plugin_manager", return_value=plugin_state()
                )
...
class TestRealWorldPluginScenarios:
    ...
    def test_plugin_upgrade_scenario(self):
        ...
        with patch(
            "fastjob.plugins.get_plugin_manager", return_value=mock_empty_manager
        ):
            load_plugin_commands()
        ...
        with patch(
            "fastjob.plugins.get_plugin_manager", return_value=mock_populated_manager
        ):
            load_plugin_commands()
