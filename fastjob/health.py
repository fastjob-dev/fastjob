...
class HealthMonitor:
    ...
    async def run_check(self, check: HealthCheck) -> Tuple[HealthStatus, str, float]:
        ...
        try:
            ...
            duration = time.time() - start_time
            return HealthStatus.UNHEALTHY, f"Check timed out after {check.timeout}s", duration
...
async def check_database_health() -> Tuple[HealthStatus, str]:
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            if tables_exist == 0:
                return HealthStatus.UNHEALTHY, "FastJob tables not found - run migrations"
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            if total_recent_jobs > 0:
                ...
                if failure_rate > 50:
                    return HealthStatus.DEGRADED, f"High failure rate: {failure_rate:.1f}%"
            
            return HealthStatus.HEALTHY, f"Job processing healthy, {recent_activity} recent updates"
...
async def check_system_resources() -> Tuple[HealthStatus, str]:
    ...
    try:
        ...
        return HealthStatus.HEALTHY, f"Resources healthy (CPU: {cpu_percent:.1f}%, Mem: {memory.percent:.1f}%, Disk: {disk.percent:.1f}%)"
...
async def cli_health_check(verbose: bool = False) -> int:
    ...
    """
    try:
        from .cli.colors import success, error, warning, print_status, print_key_value, StatusIcon
        ...
        if verbose:
            ...
            for check_name, check_result in status['checks'].items():
                ...
                critical_marker = warning(" (CRITICAL)") if check_result['critical'] else ""
                duration_info = f"({check_result['duration_ms']}ms)"
                
                print(f"  {icon} {check_name}{critical_marker}: {check_result['message']} {duration_info}")
        else:
            # Simple output
            if status['status'] == 'healthy':
                print_status(f"{status['status'].upper()}: {status['message']}", "success")
            elif status['status'] == 'degraded':
                print_status(f"{status['status'].upper()}: {status['message']}", "warning")
            else:
                print_status(f"{status['status'].upper()}: {status['message']}", "error")
