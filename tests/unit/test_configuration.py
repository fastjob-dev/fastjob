...
class TestPydanticConfiguration:
    ...
    def test_configuration_validation_invalid_database_url(self):
        ...
        with pytest.raises(ValueError, match="Invalid FastJob configuration.*database_url must be a PostgreSQL connection string"):
    ...
    def test_configuration_validation_invalid_log_level(self):
        ...
        with pytest.raises(ValueError, match="Invalid FastJob configuration.*log_level must be one of"):
            get_settings(reload=True)
    ...
    def test_configuration_validation_invalid_log_format(self):
        ...
        with pytest.raises(ValueError, match="Invalid FastJob configuration.*log_format must be one of"):
            get_settings(reload=True)
    ...
    def test_configuration_validation_numeric_ranges(self):
        ...
        with pytest.raises(ValueError, match="Invalid FastJob configuration"):
            get_settings(reload=True)
    ...
    def test_fallback_behavior_without_pydantic(self):
        ...
        with patch.dict('sys.modules', {
            ...
            # Set environment variables
            os.environ["FASTJOB_DATABASE_URL"] = "postgresql://fallback@localhost/fallback_db"
            os.environ["FASTJOB_JOBS_MODULE"] = "fallback.jobs"
            os.environ["FASTJOB_DEFAULT_CONCURRENCY"] = "6"
            os.environ["FASTJOB_LOG_LEVEL"] = "DEBUG"
            os.environ["FASTJOB_DEBUG"] = "true"
            
            # Force reload settings module to trigger fallback
            import fastjob.settings
            import importlib
            ...
            assert settings.database_url == "postgresql://fallback@localhost/fallback_db"
            assert settings.jobs_module == "fallback.jobs"
            assert settings.default_concurrency == 6
            assert settings.log_level == "DEBUG"
            assert settings.debug is True
