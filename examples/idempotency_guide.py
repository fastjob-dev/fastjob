...
async def update_user_profile_idempotent(
    user_id: int, profile_data: Dict[str, Any], version: Optional[int] = None
) -> dict:
    """
...
async def make_idempotent_api_call(
    endpoint: str, payload: Dict[str, Any], idempotency_token: str
) -> dict:
    """
...
async def aggregate_data_idempotent(
    data_source: str, time_range: str, checkpoint_key: str
) -> dict:
    """
    ...
    checkpoint = _aggregation_checkpoints.get(
        checkpoint_key,
        {"processed_batches": 0, "total_records": 0, "last_batch_id": None},
    )

    if checkpoint["processed_batches"] > 0:
        print(
            f"   📋 Resuming from checkpoint: {checkpoint['processed_batches']} batches processed"
        )
...
def generate_idempotency_key(*args, **kwargs) -> str:
    ...
    key_data = {"args": args, "kwargs": sorted(kwargs.items())}  # Sort for consistency
