...
async def clean_app():
    ...
    app = FastJob(
        database_url=os.environ.get(
            "FASTJOB_DATABASE_URL", "postgresql://postgres@localhost/fastjob_test"
        )
    )
...
async def test_notify_sent_verification(test_db, clean_app):
    ...
    try:
        ...
        assert (
            first_notification["channel"] == "fastjob_new_job"
        ), "Notification should be on fastjob_new_job channel"
        assert (
