...
class PluginManager:
        ...
        try:
            ...
            for entry_point in fastjob_plugins:
                try:
                    ...
                    logger.info(
                        f"Successfully loaded FastJob plugin: {entry_point.name} v{getattr(plugin_instance, 'version', '0.1.0')}"
                    )
            ...
            try:
                ...
                if hasattr(plugin, "register_hooks"):
                    ...
                    if hooks:
                        ...
                        logger.info(
                            f"Plugin {name} registered hooks: {', '.join(hook_names)}"
                        )
                    else:
    ...
    def call_hook(self, hook_name: str, *args, **kwargs):
        ...
        logger.debug(
            f"Calling hook '{hook_name}' with {len(hook_functions)} function(s)"
        )
        ...
        if errors:
            logger.warning(
                f"Hook '{hook_name}' completed with {len(errors)} error(s) out of {len(hook_functions)} function(s)"
            )
                ...
                try:
                    ...
                    active_plugins[name] = {
                        ...
                        "hooks_registered": len(
                            [
                                h
                                for h, funcs in self.hooks.items()
                                if any(
                                    f.__self__ == plugin
                                    for f in funcs
    ...
    def diagnose_plugins(self) -> str:
        ...
        if status["failed_plugins"]:
            ...
            for failed in status["failed_plugins"]:
                ...
                if failed.get("stage") == "initialization":
                    report.append("     Failed during: Plugin initialization")
                else:
                    report.append("     Failed during: Plugin loading")
        ...
        if status["hook_names"]:
            ...
            for hook_name in sorted(status["hook_names"]):
                ...
                report.append(
                    f"  🔌 {hook_name} ({hook_count} function{'s' if hook_count != 1 else ''})"
                )
